...
class IntcodeVM
{
    ...
    private $relativeParameterBase = 0;
    ...
    public function runUntilOutput() {
        while(true) {
            ...
            switch ($opcode) {
                ...
                case ADJUST_RELATIVE_BASE: $this->adjustRelativeBase();break;
    ...
    private function adjustRelativeBase()
    {
        $this->relativeParameterBase += $this->getArg(1);
        $this->ip += 2;
    }
    ...
    private function getArg($offset) {
        ...
        switch ($mode) {
            ...
            case PARAMETER_MODE_RELATIVE:
                $address = $this->relativeParameterBase + $this->memory[$this->ip + $offset];
    ...
    private function put($offset, $value) {
        $memoryMode = $this->parameterMode($offset);

        $address = $this->memory[$this->ip + $offset];
        
        if ($memoryMode === PARAMETER_MODE_RELATIVE) {
            $address += $this->relativeParameterBase;
        }
